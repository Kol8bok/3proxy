	...
	if [[ -e /etc/debian_version ]]; then
		...
		if [[ $ID == "debian" || $ID == "raspbian" ]]; then
			if [[ $VERSION_ID -lt 9 ]]; then
				...
				echo ""
				echo "Однако, если вы используете Debian> = 9 или unstable / testing, вы можете продолжить на свой страх и риск"
				echo ""
		...
		echo "Похоже, вы не запускаете этот установщик в системе Debian, Ubuntu, Fedora, CentOS, Amazon Linux 2, Oracle Linux 8 или Arch Linux."
		exit 1
	fi
}
...
function initialCheck() {
	if ! isRoot; then
		echo "Извините, вам нужно запустить это от root пользователя"
		exit 1
	fi
	if ! tunAvailable; then
		echo "TUN интерфейс недоступен"
		exit 1
	fi
	checkOS
}
...
function installQuestions() {
	...
	echo ""
	echo "Перед началом настройки мне нужно задать вам несколько вопросов"
	echo "Вы можете оставить параметры по умолчанию и просто нажать ввод, если они вас устраивают"
	echo ""
	echo "Мне нужно знать IPv4-адрес сетевого интерфейса, который вы хотите прослушивать OpenVPN"
	echo "Если ваш сервер не находится за NAT, это должен быть ваш общедоступный IPv4-адрес"
	

	# Detect public IPv4 address and pre-fill for the user
	IP=$(ip -4 addr | sed -ne 's|^.* inet \([^/]*\)/.* scope global.*$|\1|p' | head -1)
	IPPUB=$(wget -qO - icanhazip.com)
	echo ""
	echo "Твой локальный IP адрес: $IP" 
	echo "Твой публичный IP адрес: $IPPUB"
    echo ""
	if [[ -z $IP ]]; then
	...
	if [[ $APPROVE_IP =~ n ]]; then
		read -rp "Твой публичный IP адрес сервера: " -e -i "$IPPUB" IP
	fi
	# If $IP is a private IP address, the server must be behind NAT
	if echo "$IPPUB" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo ""
		echo "Кажется, этот сервер находится за NAT. Каков его общедоступный IPv4-адрес или имя хоста?"
		echo "Он нужен нам для подключения клиентов к серверу"
	...
	if eval "$PING6"; then
		echo "Похоже, что ваш хост поддерживает подключение по протоколу IPv6"
		SUGGESTION="y"
	else
		echo "Похоже, у вашего хоста нет возможности подключения по протоколу IPv6"
		SUGGESTION="n"
	fi
	echo ""
	...
	echo "   1) По умолчанию HTTPS: 443"
	echo "   2) Любой другой порт (default port OpenVPN 1194)"
	echo "   3) Случайный [49152-65535]"
	until [[ $PORT_CHOICE =~ ^[1-3]$ ]]; do
		read -rp "Выбери порт, опцию [1-3]: " -e -i 1 PORT_CHOICE
	done
	case $PORT_CHOICE in
	1)
		PORT="443"
	...
	2)
		until [[ $PORT =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; do
			read -rp "Укажи любой порт в диапазоне [1-65535]: " -e -i 443 PORT
		done
		;;
	3)
	...
	echo ""
	echo "Какой протокол в OpenVPN будем использовать?"
	echo "UDP протокол быстрее. TCP протокол надежнее."
	echo "   1) UDP"
	echo "   2) TCP"
	...
	echo ""
	echo "Какие сервера DNS вы хотите использовать с OpenVPN? "
	...
	until [[ $DNS =~ ^[0-9]+$ ]] && [ "$DNS" -ge 1 ] && [ "$DNS" -le 13 ]; do
		read -rp "DNS сервера [1-12]: " -e -i 3 DNS
		if [[ $DNS == 2 ]] && [[ -e /etc/unbound/unbound.conf ]]; then
			echo ""
			echo "Unbound уже установлен"
			echo "Вы можете разрешить скрипту настроить его, чтобы использовать его с ваших клиентов OpenVPN"
			echo "Мы просто добавим второй сервер в /etc/unbound/unbound.conf для подсети OpenVPN"
			echo "В текущую конфигурацию изменений не вносится"
			echo ""
			...
			until [[ $DNS1 =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
				read -rp "Первичный (основной) DNS: " -e DNS1
			done
	...
	echo ""
	echo "Вы хотите использовать сжатие? Не рекомендуется, так как это используется в атаке VORACLE."
	until [[ $COMPRESSION_ENABLED =~ (y|n) ]]; do
		read -rp"Применить компрессию (compression) [y/n]: " -e -i n COMPRESSION_ENABLED
	done
	if [[ $COMPRESSION_ENABLED == "y" ]]; then
		...
		until [[ $COMPRESSION_CHOICE =~ ^[1-3]$ ]]; do
			read -rp"Алгоритм сжатия, выберите опцию [1-3]: " -e -i 1 COMPRESSION_CHOICE
		done
	...
	echo """
	Вы хотите настроить параметры шифрования?
    Если вы не знаете, что делаете, вам следует придерживаться параметров по умолчанию, предоставленных скриптом.
    Обратите внимание: что бы вы ни выбрали, все варианты, представленные в сценарии, максимально безопасные.
	"""
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		echo "   1) AES-128-GCM "
		echo "   2) AES-192-GCM"
		echo "   3) AES-256-GCM (default)"
		echo "   4) AES-128-CBC"
		echo "   5) AES-192-CBC"
		echo "   6) AES-256-CBC"
		until [[ $CIPHER_CHOICE =~ ^[1-6]$ ]]; do
			read -rp "Шифр канала данных, выберите опцию [1-6]: " -e -i 1 CIPHER_CHOICE
		done
		...
		until [[ $CERT_TYPE =~ ^[1-2]$ ]]; do
			read -rp"Тип ключа сертификата, выберите опцию [1-2]: " -e -i 1 CERT_TYPE
		done
		case $CERT_TYPE in
		1)
			echo ""
			echo "Выберите эллиптическую кривую, которую вы хотите использовать для ключа сертификата:"
			echo "   1) prime256v1 (default)"
			echo "   2) secp384r1"
			echo "   3) secp521r1"
			until [[ $CERT_CURVE_CHOICE =~ ^[1-3]$ ]]; do
				read -rp"Эллиптическая кривая, выберите опцию [1-3]: " -e -i 1 CERT_CURVE_CHOICE
			done
		...
		1)
			...
			echo "   2) ECDHE-ECDSA-AES-256-GCM-SHA384 (default)"
		...
		2)
			...
			echo "   2) ECDHE-RSA-AES-256-GCM-SHA384 (default)"
			until [[ $CC_CIPHER_CHOICE =~ ^[1-2]$ ]]; do
				read -rp"Шифр канала управления, выберите опцию [1-2]: " -e -i 1 CC_CIPHER_CHOICE
			done
			case $CC_CIPHER_CHOICE in
			1)
		...
		until [[ $DH_TYPE =~ [1-2] ]]; do
			read -rp"Ключ Диффи-Хеллмана, выберите опцию [1-2]: " -e -i 1 DH_TYPE
		done
		case $DH_TYPE in
		1)
			echo ""
			echo "Выберите кривую, которую вы хотите использовать для ключа ECDH:"
			echo "   1) prime256v1 (default)"
			echo "   2) secp384r1"
			echo "   3) secp521r1"
		...
		2)
			...
			until [[ $DH_KEY_SIZE_CHOICE =~ ^[1-3]$ ]]; do
				read -rp "Размер ключа Диффи-Хеллмана, выберите опцию [1-3]: " -e -i 1 DH_KEY_SIZE_CHOICE
			done
		...
		until [[ $HMAC_ALG_CHOICE =~ ^[1-3]$ ]]; do
			read -rp "Алгоритм дайджеста, выбирите опцию [1-3]: " -e -i 1 HMAC_ALG_CHOICE
		done
		...
		echo "   1) tls-crypt (default)"
		echo "   2) tls-auth"
		until [[ $TLS_SIG =~ [1-2] ]]; do
			read -rp "Дополнительный механизм безопасности канала управления [1-2]: " -e -i 1 TLS_SIG
		done
	fi
	echo ""
	echo "Ладно, это все, что мне нужно. Теперь мы готовы настроить ваш сервер OpenVPN."
	echo "Вы сможете создать клиентов в конце установки."
...
client-config-dir /etc/openvpn/ccd" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s 10.0.0.0/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s 10.0.0.0/24 -o $NIC -j MASQUERADE
...
function newClient() {
	...
	echo "Имя должно состоять из буквенно-цифровых символов. Он также может включать подчеркивание или тире"
	...
	echo "   1) Нет, клиент будет без пароля"
	echo "   2) Да, защитить паролем"
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		echo ""
		echo "Указанный клиентский CN уже был найден в easy-rsa, выберите другое имя"
		exit
	else
		cd /etc/openvpn/easy-rsa/ || return
		case $PASS in
		1)
			...
			;;
		2)
			echo "⚠️Вам будет предложено ввести пароль клиента ниже ⚠️"
	...
	echo "Загрузите файл .ovpn и импортируйте его в свой клиент OpenVPN."
	exit 0
}
...
function removeOpenVPN() {
	echo "Вы действительно хотите удалить OpenVPN?"
	read -rp "Выберите опцию [y/n]: " -e -i n REMOVE
	if [[ $REMOVE == 'y' ]]; then
...
function manageMenu() {
	...
	echo ""
	echo "OpenVPN у Вас уже установлен"
	echo ""
	echo "Выберите действие"
	echo "   1) Добавить нового пользователя"
	echo "   2) Отозвать сертификат пользователя"
	echo "   3) Удалить OpenVPN и все файлы конфигурации"
	echo "   4) Выйти"
